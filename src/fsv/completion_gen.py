...
def _emit_bash(root: _Command, prog: str) -> str:
    ...
    for key, info in sorted(cases.items(), key=lambda x: -len(x[0].split())):
        lines.append(f'        "{key}")')
        # Check if prev flag has a completer
        dyn_cases = []
        stat_cases = []
        ...
        if dyn_cases or stat_cases:
            ...
            for f, completer, resource in dyn_cases:
                ...
                lines.append(f'                {f})')
                lines.append(
                    f'                    COMPREPLY=( $(FSV_THIN_COMPLETE=1 python -m fsv._complete bash {completer} {resource} "$cur"{extra}) ); return 0 ;;'
                )
            lines.append("            esac")
...
def _emit_zsh(root: _Command, prog: str) -> str:
    ...
    # Only top-level subcommands in _arguments for now
    top_subs = [s for s in root.subcommands if len(s.path) == 2]
    sub_strs = ":command:->command"
    lines.append(f"        '{sub_strs}' \\")
    lines.append("        '*:: :->args'")
    lines.append("")
    lines.append("    case $state in")
    lines.append("        command)")
    sub_list = " ".join(f"'{s.path[-1]}:{s.help}'" for s in top_subs)
    lines.append(f"            local commands=({sub_list})")
    ...
    for sub in top_subs:
        lines.append(f"                {sub.path[-1]})")
        sub_subs = [s for s in sub.subcommands if len(s.path) == 3]
        if sub_subs:
            sub_sub_list = " ".join(f"'{s.path[-1]}:{s.help}'" for s in sub_subs)
            lines.append(f"                    local sub_commands=({sub_sub_list})")
            lines.append("                    _describe 'subcommand' sub_commands")
        flags_args = []
        for p in sub.params:
            for f in p.flags:
                if p.kind in ("boolean",):
                    flags_args.append(f"'{f}[flag]'")
                else:
                    flags_args.append(f"'{f}[value]:value:'")
        if flags_args:
            lines.append("                    _arguments \\")
            for i, fa in enumerate(flags_args):
                sep = " \\" if i < len(flags_args) - 1 else ""
                lines.append(f"                        {fa}{sep}")
        lines.append("                    ;;")
